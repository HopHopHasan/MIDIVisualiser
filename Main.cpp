...
void Main()
{
    const Font fontLight{ 18, Typeface::Light };
    ...
    while (System::Update())
    {
        ...
        if (!Midi::IsPlaying())
        {
            fontLight(L" Press SPACE or CLICK to activate | SPACEキーまたはマウスをクリックすると起動します").draw(20, 20);
        ...
        if (Input::MouseL.clicked || Input::KeySpace.clicked)
        {
            Midi::Play();
        }
        ...
        for (auto& i : visibleNoteIndices)
        {
            ...
            {
                const RectF r(note.startMillisec * scale + offset - pos, (maxPitch - note.noteNumber) * blockHeight, note.lengthMillisec * scale, blockHeight);
                RoundRect(r, 5).drawShadow({ 0, 0 }, 12 + note.alpha * 8, 2 + note.alpha * 8, HSV(30 + note.ch * 100, 0.5, 1).toColorF(note.alpha * 0.6));
            }
        }
